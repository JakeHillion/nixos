...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # world-readable so the snapserver dynamic user can read it. go-librespot
    # also wants its config and state directory in place before it starts.
    systemd.tmpfiles.rules = [
      ...
      "d ${goLibrespotStateDir} 0700 go-librespot go-librespot -"
      "L+ ${goLibrespotStateDir}/config.yml - - - - ${goLibrespotConfig}"
    ];
